...
def main():
    ...
    for subId in range(len(dfSub)):
        ...
        try:
            ...
            # Configure S3 bucket
            s3_bucket_name = "damgassign02"
            s3_folder = f"Extracted_json_files/{dirname}/"   

                    # Define S3 key (path in S3)
            s3_key = f"{s3_folder}{submitted['adsh']}.json"
            ...
            if successful_files % 100 == 0:
                print(f"Uploaded {successful_files} files to S3 successfully...")

                

        except Exception as e:
            print(f"Error uploading file {submitted['adsh']} to S3: {str(e)}")
            failed_files += 1
            continue             
