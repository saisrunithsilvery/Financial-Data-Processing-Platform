...
cleaned_data AS (
    SELECT DISTINCT
        ...
        'SEC_BALANCE_SHEET' as source_file
    FROM source_data
)
...
-- -- models/staging/stg_balance_sheet.sql
-- {{ config(materialized='view') }}
...
--         s.fp as fiscal_period,  -- Keep as string, no casting needed
--         t.tlabel as metric_name,
--         n.tag as metric_code,
--         t.datatype as metric_info,
--         COALESCE(n.uom, 'USD') as measurement_unit,
--         TO_NUMBER(n.value, 38, 4) as metric_value,  -- Explicit number conversion
--         'SEC_BALANCE_SHEET' as source_file,
--         CURRENT_TIMESTAMP() as loaded_at
--     FROM {{ source('sec_data', 'num') }} n
--     INNER JOIN {{ source('sec_data', 'sub') }} s 
